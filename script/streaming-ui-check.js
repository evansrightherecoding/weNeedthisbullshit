...
event-interaction https://raw.githubusercontent.com/evansrightherecoding/weNeedthisbullshit/master/script/streaming-ui-check.js, tag=流媒体-解锁查询, img-url=checkmark.seal.system, enabled=true
...
// Correct the upstream Taiwan flag mapping for all streaming checks.
flags.set("TW", "🇹🇼")
...
function getYouTubeRegion(data) {
  ...
  if (normalizedData.indexOf('www.google.cn') !== -1) {
    return 'CN'
  }
  return null
}
...
function testYTB() { 
    ...
    $task.fetch(option).then(response=> {
      ...
      const region = getYouTubeRegion(data)
      const flag = countryCodeToFlag(region)
      //resolve(region)
      result["YouTube"] = "<b>YouTube Premium: </b>支持 "+arrow+ "⟦"+(flag || "地区未知")+"⟧ 🎉"
